...
_SOKOL_PRIVATE LRESULT CALLBACK _sapp_win32_wndproc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE:
        {
            ...
            typedef LSTATUS Win32_RegGetValueA(HKEY hkey, LPCSTR lpSubKey, LPCSTR lpValue, DWORD dwFlags, LPDWORD pdwType, PVOID pvData, LPDWORD pcbData);
            static Win32_RegGetValueA *RegGetValueA = NULL;
            HMODULE advapi32 = LoadLibraryA("Advapi32.dll");
            if (advapi32)
            {
                RegGetValueA = (Win32_RegGetValueA *)GetProcAddress(advapi32, "RegGetValueA");
            }
            ...
            if (DwmSetWindowAttribute)
            {
                ...
                if (RegGetValueA)
                {
                    status = RegGetValueA(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize", "AppsUseLightTheme", RRF_RT_ANY, NULL, &use_light_theme, &dataSize);
                }
    ...
    if (!_sapp.win32.in_create_window) {
        switch (uMsg) {
            ...
            case WM_SYSCOMMAND:
                ...
                break;
            // NOTE(nick): prevents the window from being cleared to black (BLACK_BRUSH)
            #if 0
            case WM_ERASEBKGND:
                return 0;
            #endif
...
_SOKOL_PRIVATE void _sapp_win32_create_window(void) {
    ...
    wndclassw.hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);
